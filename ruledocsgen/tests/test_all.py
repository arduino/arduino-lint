...
def test_all(run_command, working_dir):
    ...
    output_path = working_dir_path.joinpath("rules")
    result = run_command(cmd=[output_path])
    assert result.ok

    assert output_path.joinpath("sketch.md").exists
    assert "SS001" in output_path.joinpath("sketch.md").read_text()
    assert output_path.joinpath("library.md").exists
    assert "LS001" in output_path.joinpath("library.md").read_text()
    assert output_path.joinpath("platform.md").exists
    assert "PD001" in output_path.joinpath("platform.md").read_text()
    assert output_path.joinpath("package-index.md").exists
    assert "IS001" in output_path.joinpath("package-index.md").read_text()
...
def run_command(pytestconfig, working_dir) -> typing.Callable[..., invoke.runners.Result]:
    ...
    executable_path = pathlib.Path(pytestconfig.rootdir).parent / "ruledocsgen"
